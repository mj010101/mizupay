...
export function SignInMenu({ children }: Props) {
  ...
  // Filter wallets to only include Phantom
  const filteredWallets = wallets.filter((wallet) => wallet.name === "Phantom");
        ...
        <DropdownMenu.Content>
          {walletsThatSupportSignInWithSolana.length === 0 ? (
            <Callout.Root color="orange" highContrast>
              ...
              <Callout.Text>
                Phantom wallet is not installed or doesn't support{" "}
                <a
